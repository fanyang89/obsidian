...
source: "https://selfboot.cn/2024/01/17/c++_string_cow/"
author:
published:
created: 2024-11-19
...
[C++字符串修改副本影响到原来内容](https://slefboot-1251736664.file.myqcloud.com/20240115_c++_string_cow_cover.png/webp)
...
| `1234567891011121314151617181920212223242526272829` | `#include <iostream>#include <cstring>using namespace std;void ModifyStringInplace(string &str) {    size_t len = str.size();    char *s = const_cast<char *>(str.c_str());    s[0] = 'X';    return;}int main() {    string original = "Hello, World!";    string copy = original;        cout << "Original: " << original << ", address: " << static_cast<const void*>(original.c_str()) << endl;    cout << "Copy    : " <<  copy << ", address: " << static_cast<const void*>(copy.c_str()) << endl;        ModifyStringInplace(copy);        cout << "After Modification:" << endl;    cout << "Original: " << original << ", address: " << static_cast<const void*>(original.c_str()) << endl;    cout << "Copy    : " <<  copy << ", address: " << static_cast<const void*>(copy.c_str()) << endl;    return 0;}` |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |

在业务生产环境上，用 G++ 4.9.3 编译上面的代码，运行结果如下：

| `12345` | `Original: Hello, World!, address: 0x186c028Copy    : Hello, World!, address: 0x186c028After Modification:Original: Xello, World!, address: 0x186c028Copy    : Xello, World!, address: 0x186c028` |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `12345` | `Original: Hello, World!, address: 0x607028Copy    : Hello, World!, address: 0x607028After Modification:Original: Hello, World!, address: 0x607028Copy    : Xello, World!, address: 0x607058` |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

其实用 `[]` 只读取字符串中某位的内容，也会触发写时复制。比如下面的代码：

| `1234567891011121314` | `{    string original = "Hello, World!";    string copy = original;        cout << "Original: " << original << ", address: " << static_cast<const void*>(original.c_str()) << endl;    cout << "Copy    : " <<  copy << ", address: " << static_cast<const void*>(copy.c_str()) << endl;    copy[0];        cout << "After :" << endl;    cout << "Original: " << original << ", address: " << static_cast<const void*>(original.c_str()) << endl;    cout << "Copy    : " <<  copy << ", address: " << static_cast<const void*>(copy.c_str()) << endl;}` |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

在低版本 G++ 上编译运行，可以看到用 operator\[\] 读取字符串后，复制内容的地址也发生了变化(从 `0x21f2028` 到 `0x21f2058`)，如下：

| `12345` | `Original: Hello, World!, address: 0x21f2028Copy    : Hello, World!, address: 0x21f2028After Modification:Original: Hello, World!, address: 0x21f2028Copy    : Hello, World!, address: 0x21f2058` |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `123456789` | `int main() {    std::string s("str");    const char* p = s.data();    {        std::string copy = s;        std::cout << s[0] << std::endl;     }    std::cout << *p << '\n';  }` |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `1234567891011121314151617` | `int main() {    std::string thread1 = "Hello, World! This is a test string.";     std::string thread2(thread1);    std::string thread3(thread1);    std::vector<std::thread> threads;    threads.emplace_back(StringOperations, std::ref(thread1));    threads.emplace_back(StringOperations, std::ref(thread2));    threads.emplace_back(StringOperations, std::ref(thread3));    for (auto& thread : threads) {        thread.join();    }    return 0;}` |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| `1234567891011` | `#include <iostream>using namespace std;int main() {    string a = "short";    string b = "this is a long string here, hahahhh";    cout << &a << ":" << static_cast<const void*>(a.c_str()) << endl;    cout << &b << ":" << static_cast<const void*>(b.c_str()) << endl;    return 0;}` |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

用高版本编译运行，可以看到输出类似下面结果：

| `12` | `0x7ffcb9ff22d0:0x7ffcb9ff22e00x7ffcb9ff22b0:0x421eb0` |
| ---- | ------------------------------------------------------ |
